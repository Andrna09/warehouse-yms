...
const App: React.FC = () => {
  ...
  // --- AUDIO UNLOCKER HELPER ---
  const unlockAudio = () => {
    // Memainkan suara silent super pendek untuk "memancing" browser membuka izin audio
    ...
    if (AudioContext) {
        ...
        gain.gain.value = 0.00001; // Hampir silent
        osc.start(0);
        osc.stop(0.1);
    }
    // Backup untuk iOS Safari
    const silentAudio = new Audio("data:audio/mp3;base64,//MkxAA........"); // Empty MP3 base64 or just dummy play
    silentAudio.play().catch(() => {}); 
  };

  // Helper untuk Navigasi dengan Audio Unlock
  const navigateWithUnlock = (targetView: string) => {
      ...
      <div className="w-full max-w-[1440px] grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
        ...
        <div className="lg:col-span-5 text-left space-y-8 pl-4">
          ...
          <div className="flex flex-col sm:flex-row gap-5 pt-4 animate-fade-in-up opacity-0" style={{animationDelay: '0.4s'}}>
            {/* Primary Button */}
            <button 
              onClick={() => navigateWithUnlock('public-monitor')}
            ...
            {/* Secondary Button */}
            <button 
              onClick={() => navigateWithUnlock('checkin')}
  ...
  const handleCheckInSuccess = (id: string) => {
    ...
    // Form submission is a user interaction, so audio is unlocked here automatically
    setView('status');
  };
